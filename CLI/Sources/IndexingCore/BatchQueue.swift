...
public actor BatchQueue {
    ...
    public func prioritize(_ id: Int) {
    ...
    /// progress ticker.
    public func snapshotDoneCount() -> Int { doneCount }

    /// Inspect a batch's current state. Mainly for tests.
    public func state(of id: Int) -> State? { states[id] }

    /// Count of batches with the given state. Mainly for tests.
    public func count(in state: State) -> Int {
