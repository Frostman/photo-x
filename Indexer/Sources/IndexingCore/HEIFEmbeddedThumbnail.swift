...
public enum HEIFEmbeddedThumbnail {
    ...
    public struct ExtractedThumbnail: Equatable {
        public let jpeg: Data
        public let exifOrientation: Int     // 1, 3, 6, or 8
        public let exifBytes: Data?         // TIFF block, ready for TIFFEXIFParser
        public init(jpeg: Data, exifOrientation: Int, exifBytes: Data?) {
            self.jpeg = jpeg
            self.exifOrientation = exifOrientation
            self.exifBytes = exifBytes
        }
    }
    ...
    public static func extract(from url: URL) throws -> ExtractedThumbnail? {
    ...
    public static func locateExifItem(in data: Data) -> ItemLocation? {
    ...
    public static func locateJPEGThumbnail(in data: Data) -> ItemLocation? {
        var cursor = BoxCursor(data: data, base: 0, end: data.count)
        while let box = cursor.next() {
            if box.type == "meta" {
    ...
    public static func locateOrientation(in data: Data) -> Int? {
        var cursor = BoxCursor(data: data, base: 0, end: data.count)
    ...
    public static func readU64(_ data: Data, at offset: Int) -> UInt64 {
        let base = data.startIndex + offset
        var v: UInt64 = 0
        for i in 0 ..< 8 {
    ...
    public static func readVarUInt(_ data: Data, at offset: Int, size: Int) -> UInt64 {
        let base = data.startIndex + offset
        var v: UInt64 = 0
        for i in 0 ..< size {
            v = (v << 8) | UInt64(data[base + i])
    ...
    public static func asciiString(_ data: Data, offset: Int, length: Int) -> String {
        let base = data.startIndex + offset
